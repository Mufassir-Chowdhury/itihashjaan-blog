    ...
    <article class="relative overflow-hidden rounded-lg border border-amber-100 bg-black shadow-sm transition-all duration-200 hover:shadow-md h-full">
      <div class="aspect-video {featured ? 'aspect-[16/9]' : 'aspect-[16/9]'} w-full overflow-hidden">
        <img
          ...
          alt={post.title}
          class="h-full w-full object-cover transition-transform duration-200 group-hover:scale-105 group-hover:opacity-80 bg-black"
        />
        <!-- Gradient overlay -->
      ...
      <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
        ...
        <h2 class="mb-2 text-xl {featured ? 'text-2xl md:text-3xl' : ''} font-bold group-hover:text-amber-200">
          {post.title}
        </h2>
        
        {#if featured}
          <p class="mb-4 text-gray-200 line-clamp-3">
            {post.description}
          </p>
        {/if}
